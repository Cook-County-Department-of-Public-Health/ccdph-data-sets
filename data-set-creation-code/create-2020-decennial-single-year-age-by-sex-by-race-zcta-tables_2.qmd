...
for (agroup in grouplist) {
  for (azcta_partial in  zctas_cook_county_partial){
      ...
      aregion = paste0("zip code tabulation area (or part):", as.character(azcta_partial))
      aregionin = paste0("state:17")
      print(agroupname)
      print(aregionin)
...
munis_nopartial_single_age_sex_race_2020 <- munis_total_single_age_sex_race_2020 %>% 
...
munis_withpartial_single_age_sex_race_2020 <- munis_nopartial_single_age_sex_race_2020 %>% bind_rows(munis_partial_single_age_sex_race_2020)
...
munis_withpartial_single_age_sex_race_2020_attr <- munis_withpartial_single_age_sex_race_2020 %>%
...
munis_total_single_age_sex_race_2020_attr <- munis_total_single_age_sex_race_2020 %>%
  left_join(group_categories, by = "census_group") %>%
  pivot_longer(cols=c(age_001N:age_209N), names_to = "age_sex", values_to = "population") %>%
  mutate(col_number = as.integer(substr(age_sex,5,7)),
         sex = case_when(col_number == 1 ~ "Total",
...
munis_comp_single_age_sex_race_2020_attr <- munis_withpartial_single_age_sex_race_2020_attr %>%
  left_join(munis_total_single_age_sex_race_2020_attr %>% select(geoid_place,name_age_1yr,sex,race,pop_total = population), by=c("geoid_place", "name_age_1yr", "sex","race")) %>%
...
dbWriteTable(conn = con_inter_census, 
               ...
               overwrite= TRUE,
               munis_comp_single_age_sex_race_2020_attr)

write_csv(munis_comp_single_age_sex_race_2020_attr, "C:/Users/christopher.smith/OneDrive - Cook County Health/git_repos/justenvirons/ccdph-data-sets/2020/decennial-2020-single-year-age-sex-race-by-muni.csv")
