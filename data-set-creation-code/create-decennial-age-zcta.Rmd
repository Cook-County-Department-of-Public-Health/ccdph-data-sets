...
## Pull table PCT12 (sex by single year age) from decennial for all Cook ZCTAs; Total Pop and Pop in Cook

```{r zcta_age_single}

#Create empty list to store place data
dec_age_pops_single_zcta_list <- list()

#loop through zcta list to pull data
for (i in 1:length(cook_zctas)) {
  
  api <- paste0(census_api,"?get=NAME,group(PCT12)&for=zip%20code%20tabulation%20area:", cook_zctas[[i]], "&key=", key_get("census-api-key"))
  ...
  #add results to list
  dec_age_pops_single_zcta_list[[i]] <- zcta_pull
  
}

#combine separate town files into one
dec_age_pops_single_zcta <- bind_rows(dec_age_pops_single_zcta_list)

#clean file
dec_age_pops_single_zcta_clean <- dec_age_pops_single_zcta %>%
  filter(!grepl(".*:$", label)) %>%  #remove total pop, total male, total female 
  mutate(label = trimws(str_remove(label, "!!Total:!!"))) %>%
  separate(label, into = c("sex", "age"), sep = ":!!") %>%
  mutate(geoid_zcta = as.numeric(gsub("^.* ", "", NAME))) %>%
...
#Create empty list to store place data - cook
dec_age_pops_single_zcta_list_cook <- list()

#loop through zcta list to pull data - cook
for (i in 1:length(cook_zctas)) {
  
  api <- paste0(census_api, "?get=NAME,group(PCT12)&for=county%20(or%20part):031&in=state:17%20zip%20code%20tabulation%20area%20(or%20part):", cook_zctas[[i]], "&key=", key_get("census-api-key"))
  ...
  #add results to list
  dec_age_pops_single_zcta_list_cook[[i]] <- zcta_pull
  
}

#combine separate files into one - cook
dec_age_pops_single_zcta_cook <- bind_rows(dec_age_pops_single_zcta_list_cook) %>%
  mutate(geoid_zcta = as.numeric(str_extract(NAME,"(?<=ZCTA5 ).*(?=, Illinois)"))) %>%
  select(geoid_zcta, variable, pop_in_cook)

#create final file
dec_age_pops_single_zcta_clean_final <- dec_age_pops_single_zcta_clean %>%
  left_join(dec_age_pops_single_zcta_cook)

#check numbers
total_pop_check <- read_csv("https://github.com/Cook-County-Department-of-Public-Health/ccdph-data-sets/blob/main/2020/decennial-2020-total-zcta.csv?raw=TRUE") %>%
  pull(total_pop) %>%
  sum()

total_cook_check <- read_csv("https://github.com/Cook-County-Department-of-Public-Health/ccdph-data-sets/blob/main/2020/decennial-2020-total-zcta.csv?raw=TRUE") %>%
  pull(pop_in_cook) %>%
  sum()

sum(dec_age_pops_single_zcta_clean_final$pop_in_cook)
sum(dec_age_pops_single_zcta_clean_final$total_pop)

#save
write_csv(dec_age_pops_single_zcta_clean_final, paste0(getwd(),"/2020/decennial-2020-single-year-age-sex-by-zcta.csv"))
...
## Pull table P5 (ethnicity by race) from decennial for all Cook ZCTAs; Total Pop and Pop in Cook
...
race_vars <- dec_vars %>%
  filter(grepl("P5.*N$", name)) %>%  #get all race/eth vars
  filter(!grepl("!!Total:!!Hispanic or Latino:!!", label)) %>%  #filter out Hispanic sub-categories
  filter(!grepl("001|002", name))  %>% #filter out totals so only mutually exclusive remain
...
for (i in 1:length(cook_zctas)) {
  ...
  api <- paste0(census_api,"?get=NAME,group(P5)&for=zip%20code%20tabulation%20area:", cook_zctas[[i]], "&key=", key_get("census-api-key"))
...
for (i in 1:length(cook_zctas)) {
  ...
  api <- paste0(census_api, "?get=NAME,group(P5)&for=county%20(or%20part):031&in=state:17%20zip%20code%20tabulation%20area%20(or%20part):", cook_zctas[[i]], "&key=", key_get("census-api-key"))
...
sum(dec_race_pops_zcta_clean_final$pop_in_cook)
