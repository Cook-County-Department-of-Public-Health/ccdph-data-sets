...
subtitle: "<u>2020 DECENNIAL DATA BY SINGLE-YEAR AGE, SEX, RACE, ZCTA</u>: <i>Steps for creating 2020 decennial population counts for rate denominators</i>"
...
zctas_partial_single_age_sex_race_2020 <- bind_rows(alist_pattern)  %>%
  rename(geoid_zcta = zip_code_tabulation_area_or_part) %>%
  select(-c(county_or_part, GEO_ID, NAME, state))

# remove partial zctas tables from environment
rm(list=ls(pattern="_zcta_partial"))
...
for (agroup in grouplist) {
      ...
      census_group <- census_group %>%
        ...
        mutate(census_group = agroup,
               ...
               source = "https://api.census.gov/data/2020/dec/dhc") %>%
        rename_with(~str_replace(.,agroup,"age"))
      assign(paste(agroup,"zctas_all", ayear,sep="_"),census_group)
  }
...
zctas_total_single_age_sex_race_2020 <- bind_rows(alist_pattern) %>%
  rename(geoid_zcta = zip_code_tabulation_area_or_part) %>%  
...
zctas_nopartial_single_age_sex_race_2020 <- zctas_total_single_age_sex_race_2020 %>%
  anti_join(zctas_partial_single_age_sex_race_2020, by="geoid_zcta")
...
zctas_withpartial_single_age_sex_race_2020 <- zctas_nopartial_single_age_sex_race_2020 %>% bind_rows(zctas_partial_single_age_sex_race_2020)
...
# reformat partial and non-partial zcta table
zctas_withpartial_single_age_sex_race_2020_attr <- zctas_withpartial_single_age_sex_race_2020 %>%
...
zctas_total_single_age_sex_race_2020_attr <- zctas_total_single_age_sex_race_2020 %>%
  left_join(group_categories, by = "census_group") %>%
  pivot_longer(cols=c(age_001N:age_209N), names_to = "age_sex", values_to = "population") %>%
  mutate(col_number = as.integer(substr(age_sex,5,7)),
         sex = case_when(col_number == 1 ~ "Total",
...
zctas_comp_single_age_sex_race_2020_attr <- zctas_withpartial_single_age_sex_race_2020_attr %>%
  left_join(zctas_total_single_age_sex_race_2020_attr %>% select(geoid_zcta,name_age_1yr,sex,race,pop_total = population), by=c("geoid_zcta", "name_age_1yr", "sex","race")) %>%
  select(geoid_zcta:race,pop_in_cook,pop_total,vintage,source)
...
dbWriteTable(conn = con_inter_census, 
               Id(schema="ref", table="decennial-2020-single-year-age-sex-race-ethnicity-by-zcta"), 
               overwrite= TRUE,
               zctas_comp_single_age_sex_race_2020_attr)

write_csv(zctas_comp_single_age_sex_race_2020_attr, "C:/Users/christopher.smith/OneDrive - Cook County Health/git_repos/justenvirons/ccdph-data-sets/2020/decennial-2020-single-year-age-sex-race-by-zcta.csv")
