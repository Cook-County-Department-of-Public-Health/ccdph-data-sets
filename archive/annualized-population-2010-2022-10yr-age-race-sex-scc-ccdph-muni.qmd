...
# Join SCC total population data with standard age groups
standard_age_groups_seer_pop <- standard_age_groups_seer %>% 
  left_join(pop_2010_scc_muni_10yr, by="order_age_10yr") %>% 
   left_join(pop_2020_scc_muni_10yr, by="order_age_10yr")
...
assign(value = atable, x = "standard_age_groups_seer_pop_10yr_annualized")
standard_age_groups_seer_pop_10yr_annualized <- standard_age_groups_seer_pop_10yr_annualized %>% 
  relocate(pop_2010, .before = pop_2011) %>% 
  relocate(pop_2020, .after = pop_2019) %>% 
  pivot_longer(cols = pop_2010:pop_2030,
               names_to = "pop_year",
...
standard_age_groups_seer_10yr_annualized_all <- standard_age_groups_seer_pop_10yr_annualized %>%
  mutate(stratification = "total") %>% 
  bind_rows(
    standard_age_groups_seer_pop_race_10yr_annualized %>%
      rename(stratification = race)
    ) %>%
  bind_rows(
    standard_age_groups_seer_pop_sex_10yr_annualized %>%
      rename(stratification = sex)
    ) %>% 
  mutate(geoid = "scc") %>% 
